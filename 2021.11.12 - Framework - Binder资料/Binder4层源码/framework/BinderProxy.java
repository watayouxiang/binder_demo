...
package com.maniu.mmkvdemo.test.framework;
...
public final class BinderProxy implements android.os.IBinder {
    ...
    /**
     * @see {@link android.os.Binder#setProxyTransactListener(listener)}.
     */
    public static void setTransactListener(@Nullable android.os.Binder.ProxyTransactListener listener) {
    ...
    private static final class ProxyMap {
        ...
        void set(long key, @NonNull BinderProxy value) {
            ...
            if (size >= mWarnBucketSize) {
                ...
                Log.v(android.os.Binder.TAG, "BinderProxy map growth! bucket size = " + size
                        + " total = " + totalSize);
                ...
                if (Build.IS_DEBUGGABLE && totalSize >= CRASH_AT_SIZE) {
                    ...
                    if (totalUnclearedSize >= CRASH_AT_SIZE) {
                        ...
                        Log.v(android.os.Binder.TAG, "BinderProxy map has many cleared entries: "
        ...
        private void dumpProxyInterfaceCounts() {
            ...
            Log.v(android.os.Binder.TAG, "BinderProxy descriptor histogram "
                    + "(top " + Integer.toString(MAX_NUM_INTERFACES_TO_DUMP) + "):");
            for (int i = 0; i < sorted.length; i++) {
                Log.v(android.os.Binder.TAG, " #" + (i + 1) + ": " + sorted[i]);
            }
        }
        ...
        private void dumpPerUidProxyCounts() {
            ...
            Log.d(android.os.Binder.TAG, "Per Uid Binder Proxy Counts:");
            for (int i = 0; i < counts.size(); i++) {
                final int uid = counts.keyAt(i);
                final int binderCount = counts.valueAt(i);
                Log.d(android.os.Binder.TAG, "UID : " + uid + "  count = " + binderCount);
            }
        }
    ...
    public boolean transact(int code, Parcel data, Parcel reply, int flags) throws RemoteException {
        ...
        if (mWarnOnBlocking && ((flags & FLAG_ONEWAY) == 0)) {
            ...
            Log.w(android.os.Binder.TAG, "Outgoing transactions from this process must be FLAG_ONEWAY",
                    new Throwable());
        }

        final boolean tracingEnabled = android.os.Binder.isTracingEnabled();
        if (tracingEnabled) {
            final Throwable tr = new Throwable();
            android.os.Binder.getTransactionTracker().addTrace(tr);
        ...
        final android.os.Binder.ProxyTransactListener transactListener = sTransactListener;
        Object session = null;

        if (transactListener != null) {
            final int origWorkSourceUid = android.os.Binder.getCallingWorkSourceUid();
    ...
    /**
     *  See {@link android.os.IBinder#getInterfaceDescriptor()}
     */
    ...
    /**
     * See {@link android.os.IBinder#linkToDeath(DeathRecipient, int)}
     */
    public native void linkToDeath(DeathRecipient recipient, int flags)
            throws RemoteException;
    /**
     * See {@link android.os.IBinder#unlinkToDeath}
     */
